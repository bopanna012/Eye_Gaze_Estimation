...
# The shape predictor and sound files live in the repo root, two directories
# above this script.
REPO_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

# Load sounds
sound = pyglet.media.load(os.path.join(REPO_ROOT, "sound.wav"), streaming=False)
left_sound = pyglet.media.load(os.path.join(REPO_ROOT, "left.wav"), streaming=False)
right_sound = pyglet.media.load(os.path.join(REPO_ROOT, "right.wav"), streaming=False)
...
predictor = dlib.shape_predictor(os.path.join(REPO_ROOT, "shape_predictor_68_face_landmarks.dat"))
...
def draw_letters(letter_index, text, letter_light):
    ...
    if letter_light is True:
        ...
        cv2.rectangle(keyboard, (x + th, y + th), (x + width - th, y + height - th), (51, 51, 51), -1)#gray
        cv2.putText(keyboard, text, (text_x, text_y), font_letter, font_scale, (255, 255, 255), font_th)#white
